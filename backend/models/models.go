...
type AiAssistantSession struct {
	...
	Title     string    `json:"title" gorm:"size:120"`          // 会话标题，保存时由首条 user 消息自动派生
	CreatedAt time.Time `json:"createdAt"`
	UpdatedAt time.Time `json:"updatedAt"`
...
// AiAssistantSessionSummary 会话列表项（不含完整 messages，避免列表接口 payload 过大）
type AiAssistantSessionSummary struct {
...
type AiAssistantMessage struct {
	...
	Steps        []string        `json:"steps,omitempty"`       // 执行步骤（✳/🔧 等带前缀的步骤行）
	JsonMarkdown string          `json:"jsonMarkdown,omitempty"` // 从 JSON 响应体提取的「分析报告」Markdown
