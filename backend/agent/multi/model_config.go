...
// Loads the global SettingConfig, finds the AIConfig matching aiConfigID,
// and creates the model via the same routing logic as backend/agent/chat_model_factory.go.
// Falls back to the first available AIConfig when the ID is not found.
func GetChatModel(ctx context.Context, role string, aiConfigID int) (model.ToolCallingChatModel, error) {
	...
	// Fall back to the first available AI config
	if aiCfg == nil && len(cfg.AiConfigs) > 0 {
		aiCfg = cfg.AiConfigs[0]
	}
	if aiCfg == nil {
		return nil, fmt.Errorf("no AI config found (aiConfigID=%d)", aiConfigID)
	}

	logger.SugaredLogger.Infof("GetChatModel role=%q aiConfigID=%d model=%q base=%q",
		role, aiConfigID, aiCfg.ModelName, aiCfg.BaseUrl)

	return createChatModelFromConfig(ctx, aiCfg)
}

// createChatModelFromConfig mirrors the logic in backend/agent/chat_model_factory.go:createChatModel.
// Routes to the appropriate Eino model provider based on the base URL and model name.
func createChatModelFromConfig(ctx context.Context, aiConfig *data.AIConfig) (model.ToolCallingChatModel, error) {
	...
	logger.SugaredLogger.Infof("createChatModelFromConfig provider=%d base=%q model=%q", p, aiConfig.BaseUrl, aiConfig.ModelName)
...
// ---------------------------------------------------------------------------
// Helpers — mirrors of helpers in backend/agent/chat_model_factory.go
// ---------------------------------------------------------------------------
...
// buildProxyHTTPClient builds an HTTP client with proxy, or returns nil if proxy is not configured.
