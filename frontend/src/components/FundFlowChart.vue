...
const props = defineProps({
  ...
  flowType: {
    type: String,
    default: 'bk',
    validator: (v: string) => ['bk', 'concept'].includes(v)
  }
})
...
const FLOW_CONFIG = {
  bk: {
    label: '板块',
    getAllCodes: marketApi.getAllBKCodes,
    getTopList: marketApi.getBKFundFlowTopListByDate,
    getFlowList: marketApi.getBKFundFlowListByDate,
  },
  concept: {
    label: '概念',
    getAllCodes: marketApi.getAllConceptCodes,
    getTopList: marketApi.getConceptFundFlowTopListByDate,
    getFlowList: marketApi.getConceptFundFlowListByDate,
  }
} as const

type FlowType = keyof typeof FLOW_CONFIG

const config = computed(() => FLOW_CONFIG[props.flowType as FlowType])
...
onMounted(async () => {
  try {
    // 加载所有代码（供临时添加使用）
    const codes = (await config.value.getAllCodes()).data
    if (codes && Array.isArray(codes)) allCodes.value = codes
...
async function loadAllData() {
  ...
  try {
    ...
    // 按日期获取排名
    const res = (await config.value.getTopList(date, 500)).data
    ...
    const allData = await Promise.all(
      sectors.map(async (item: any) => {
        const points = (await config.value.getFlowList(item.code, date)).data
        return {
          code: item.code,
          name: item.name,
...
function renderChart(allData: { code: string; name: string; isInflow: boolean; points: any[] }[], fullRender: boolean = false) {
  ...
  const option: echarts.EChartsOption = {
    ...
    title: {
      text: `${dateLabel} ${label}资金流向 - 多${label}对比`,
      left: '20px',
...
function addExtraSector(code: string) {
  if (!code) return
  const found = allCodes.value.find((i: any) => i.code === code)
  if (!found) return
