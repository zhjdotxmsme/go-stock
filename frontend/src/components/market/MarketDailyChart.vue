...
watch(() => props.data, (newData) => {
  if (newData && newData.length > 0) {
    renderDailyUpDownChart(newData)
    renderDailyLimitChart(newData)
  }
}, { immediate: true })
...
function renderDailyUpDownChart(rawData) {
  const data = aggregateByDate(rawData)
  ...
  const option = {
    ...
    series: [
      { name: '上涨家数', type: 'bar', data: upCounts, itemStyle: { color: '#ef4444' } },
      { name: '下跌家数', type: 'bar', data: downCounts, itemStyle: { color: '#22c55e' } },
      {
...
function renderDailyLimitChart(rawData) {
  const data = aggregateByDate(rawData)
  ...
  const option = {
    ...
    series: [
      { name: '涨停家数', type: 'bar', data: limitUps, itemStyle: { color: '#ef4444' } },
      { name: '跌停家数', type: 'bar', data: limitDowns, itemStyle: { color: '#22c55e' } },
      {
