...
| ✅ P3-b-2（A1 step 1） | 注册表一致性契约：TestRegistryParity 使两套注册表漂移即红灯（发现并统一同能力异名工具 QueryStockNewsTool→QueryStockNews；20 个 eino 独有 MCP/技能管理工具与 4 个旧链分页变体显式白名单化）；TestGroupTaxonomyParity 钉住核心分组路由。A1 step 2（AskAiWithToolsDepth 调度改为 executor 随定义，再从 eino ToolInfo 生成 schema）需先重构流式 ToolContext 状态归属，独立排期 | step 1 ✅（b2e8cc9） |
