...
| ✅ P3-b-1（P1-5 增量迁移） | 新增 Snapshot 数据域（port DTO + 腾讯全字段/东财价格双 provider + Router.GetSnapshot：singleflight/15s 缓存/8s 超时）；**第1组** GetStockRealTimePrice 迁移 Router（离线回退链矩阵 + 新旧契约等价测试 + 在线同源对照：名称精确一致、价格零偏差）；**第2组** GetStockKLineWithFallback 的 klt=101 日线迁移 Router（KLineData 增加 Source 字段；barsToLegacyKLineResult 反向转换器；ConvertKLineData 补 TDX `2006-01-02 15:04` 布局；在线对照：日期与收盘完全一致）；附带修复 SettingConfig 内嵌 `*Settings` 空指针（nil-DB 下访问提升字段必 panic 的根因） | ✅ 已完成（0784a6a、09795c7） |
| ✅ P3-b-2（A1 step 1） | 注册表一致性契约：TestRegistryParity 使两套注册表漂移即红灯（发现并统一同能力异名工具 QueryStockNewsTool→QueryStockNews；20 个 eino 独有 MCP/技能管理工具与 4 个旧链分页变体显式白名单化）；TestGroupTaxonomyParity 钉住核心分组路由。剩余（A1 step 2）：AskAiWithToolsDepth 调度改为 executor 随定义（旧链 handler 携带流式 ToolContext 状态，需先重构调度器再从 eino ToolInfo 生成 schema） | step 1 ✅（b2e8cc9），step 2 独立排期 |
| ✅ P3-b-3（S4 step 1） | DailyPickService 剥离 Wails runtime：注入式 `ProgressEmitter`（nil 安全，服务不再 import wails）；新建 DailyPickHandler 承接 Wails 绑定与事件桥接（13 个方法全量透传，前端契约不变，绑定路径 data.DailyPickService → handler.DailyPickHandler，wailsjs 绑定与前端导入已同步更新）。剩余（S4 step 2）：服务内 db.Dao 直查抽 repository（约2800行引擎内部，属完整功能迁移） | step 1 ✅，step 2 独立排期 |
