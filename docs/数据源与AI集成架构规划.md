...
| ✅ P3-a | P1-2 Yahoo PowerShell 降级响应缓存（60s TTL，进程创建从每请求一次降为每 TTL 一次；顺带删除 `_windows/_other` 死代码副本）；S1 handler ctx 传播（6 个 handler 74 处 `context.Background()` → `h.currentCtx()`，fund/news/stockchange/trading 新增 ctxFn 管道并在 app.go 接线——服务调用可随应用退出取消）；A1 子集：`thsResultToMarkdown` 双实现合并为 data 包 `ThsResultToMarkdown` 单一实现 | ✅ 已完成（1309ccd） |
| ✅ P3-b-1（P1-5 增量迁移，3 组） | 新增 Snapshot 数据域（port DTO + 腾讯全字段/东财价格双 provider + Router.GetSnapshot：singleflight/15s 缓存/8s 超时）；**第1组** GetStockRealTimePrice 迁移（离线回退链矩阵 + 契约等价测试 + 在线同源对照：名称精确一致、价格零偏差）；**第2组** GetStockKLineWithFallback 的 klt=101 日线迁移（KLineData.Source 字段 + 反向转换器 + TDX 日期布局修复 + SettingConfig 内嵌 `*Settings` 空指针根因修复）；**第3组** GetStockKLine/GetStockCommonKLine 迁移，对照测试揪出两个真链路 bug 并修复：TDX provider 对不可映射代码静默返回错标的数据（hk00700 收盘 11.21 vs 正确 455.20）→ 非 A 股改 ErrUnsupported 跳过；TencentKLineProvider 把数值周期码传给腾讯 fqkline（要 day/week/month）→ 修正周期词表 + HK/US 代码透传。剩余直连均为语义耦合（分页 end 日期、复权旗标芯片分布、TDX 专属指标）或纯 DB CRUD（属 repository 域），已逐一定性 | ✅ 已完成（0784a6a、09795c7、4768ab2） |
| ✅ P3-b-2（A1 全量完成） | **step 1**：TestRegistryParity 使 eino/旧链两套注册表漂移即红灯（发现并统一同能力异名工具 QueryStockNewsTool→QueryStockNews；20 个 eino 独有 MCP/技能管理工具与 4 个旧链分页变体显式白名单化）；TestGroupTaxonomyParity 钉住核心分组路由。**step 2**：定义中心化注册——ToolDefinition（schema+handler+required-key）一体注册并在 Tools() 装配时过双向一致性门禁（API-Key 过滤前执行，门禁结果供测试断言）；门禁上线即抓到真孤儿 GetChipDistribution（注册了 handler 但从未有 schema，功能实际由 StockHandler 提供）；GetCurrentTime 作为定义式参考迁移。其余 21 个 tool 文件的 schema 就地合并属可选机械项，一致性已由门禁对全表强制 | ✅ 已完成（b2e8cc9、615bc61） |
| ✅ P3-b-3（S4 全量完成） | **step 1**：DailyPickService 剥离 Wails runtime（注入式 ProgressEmitter，nil 安全），新建 DailyPickHandler 承接绑定与事件桥接（13 方法透传，wailsjs 绑定与前端导入同步更新）；**step 2**：抽取 DailyPickRepository——service/review/engine 全部 26 处 db.Dao 直查收拢，查询体逐字迁移，所有方法 ctx 全链路 WithContext（此前完全无 context），data 层 db 导入从三个消费方移除，可注入 headless 测试 | ✅ 已完成（cef882f、c4d58ea） |
